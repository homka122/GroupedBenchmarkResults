...
RESULT_ROOT = Path("results2/results/homka_bgef_depth_g0")
DEFAULT_OUTPUT = Path("analysis2/png/homka_bgef_depth_not_grouped_time_table.png")
DEFAULT_CSV = Path("analysis2/tables/homka_bgef_depth_not_grouped_time_table.csv")
BENCH_ORDER = [
    "basic",
    "collections",
    "cornerCases",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Create a not-grouped running-time heatmap table for homka_bgef_depth_g0."
    )
    parser.add_argument(
        "--output",
        type=Path,
...
def parse_result_file(path: Path) -> ResultRow | None:
    match = re.search(r"D(\d+) C(\d+).*G0", path.parent.name)
    if not match:
        return None
    ...
    if len(df) == 3 and ok.all():
        return ResultRow(depth, contexts, bench, float(time_sec.mean()), "OK")
...
def load_results(root: Path) -> pd.DataFrame:
    ...
    if not rows:
        raise SystemExit(f"No not-grouped result files found in {root}.")
...
def choose_cell(cell: pd.DataFrame) -> tuple[float | None, str]:
    ...
    successful = cell[(cell["status"] == "OK") & cell["time_sec"].notna()]
    if not successful.empty:
        value = float(successful.iloc[0]["time_sec"])
    ...
    for bench in ordered_benches(df):
        for depth in depths:
            for context in contexts:
                ...
                time_sec, label = choose_cell(cell)
                rows.append(
                    {
                        "bench": bench,
                        "depth": depth,
...
def plot_table(df: pd.DataFrame, output: Path) -> None:
    ...
    ok_values = df.loc[df["status"] == "OK", "time_sec"].dropna()
    if ok_values.empty:
        raise SystemExit("No successful time values found.")
    ...
    for row_idx, bench in enumerate(benches):
        for col_idx, (depth, context) in enumerate(columns):
            ...
            if time_sec is not None:
                value_matrix[row_idx, col_idx] = time_sec
    ...
    fig.suptitle("Not-grouped running time", fontsize=16, fontweight="bold", y=0.98)
    ...
    cbar.set_label("within-benchmark time scale")
    fig.text(
        0.5,
        0.06,
        "Cells show mean running time over 3 successful not-grouped runs. '-' means the benchmark was not run in this configuration.",
        ha="center",
        fontsize=9,
        color="#475569",
    )
