...
DEFAULT_OUTPUT = Path("analysis2/png/homka_bgef_depth_not_grouped_memory_table.png")
DEFAULT_CSV = Path("analysis2/tables/homka_bgef_depth_not_grouped_memory_table.csv")
BENCH_ORDER = [
    "basic",
    "collections",
    "cornerCases",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Create a not-grouped memory-usage heatmap table for homka_bgef_depth_g0."
    )
    parser.add_argument(
        "--output",
        type=Path,
...
def plot_table(df: pd.DataFrame, output: Path) -> None:
    ...
    fig.suptitle("Not-grouped memory usage", fontsize=16, fontweight="bold", y=0.98)
    ...
    fig.text(
        0.5,
        0.06,
        "Cells show mean RAM over the last 2 successful not-grouped runs. '-' means the benchmark was not run in this configuration.",
        ha="center",
        fontsize=9,
        color="#475569",
    )
