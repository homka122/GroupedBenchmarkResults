...
DEFAULT_OUTPUT = Path("analysis2/png/homka_bgef_depth_grouped_memory_ratio_table.png")
DEFAULT_CSV = Path("analysis2/tables/homka_bgef_depth_grouped_memory_ratio_table.csv")
BENCH_ORDER = [
    "basic",
    "collections",
    "cornerCases",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Create a grouped/not-grouped memory ratio heatmap table for homka_bgef_depth."
    )
    parser.add_argument(
        "--output",
        type=Path,
...
def parse_result_file(path: Path, variant: str, group: int) -> ResultRow | None:
    ...
    if ok.sum() >= 2:
        return ResultRow(variant, depth, contexts, bench, float(ram_kb[ok].tail(2).mean()), "OK")
...
def select_result(cell: pd.DataFrame) -> tuple[float | None, str]:
    if cell.empty:
        return None, "-"
    row = cell.iloc[0]
    if row["status"] == "OK" and pd.notna(row["ram_kb"]):
        return float(row["ram_kb"]), "OK"
    return None, str(row["status"])


def choose_cell(cell: pd.DataFrame) -> tuple[float | None, str, str]:
    grouped_ram, grouped_status = select_result(cell[cell["variant"] == "grouped"])
    not_grouped_ram, not_grouped_status = select_result(cell[cell["variant"] == "not_grouped"])
    ...
    if grouped_status == "OOT" and not_grouped_status == "OOT":
        return None, "OOT", "oot"
    if grouped_status == "OOT" or not_grouped_status == "OOT":
        return None, "-", "one_oot"

    if grouped_ram is not None and not_grouped_ram is not None:
        ratio = not_grouped_ram / grouped_ram
        return ratio, format_ratio(ratio), "exact"
    ...
    for bench in ordered_benches(df):
        for depth in depths:
            for context in contexts:
                ...
                ratio, label, status = choose_cell(cell)
                rows.append(
                    {
                        "bench": bench,
                        "depth": depth,
                        "contexts": context,
                        "memory_ratio_not_grouped_over_grouped": ratio,
                        "cell": label,
                        "status": status,
                    }
                )
...
def plot_table(df: pd.DataFrame, output: Path) -> None:
    ...
    for row_idx, bench in enumerate(benches):
        for col_idx, (depth, context) in enumerate(columns):
            ...
            if ratio is not None and np.isfinite(ratio) and ratio > 0:
                value_matrix[row_idx, col_idx] = ratio
                color_matrix[row_idx, col_idx] = np.log2(ratio)

    finite = color_matrix[np.isfinite(color_matrix)]
    if len(finite) == 0:
        raise SystemExit("No comparable grouped/not-grouped memory ratio values found.")
    ...
    cmap = LinearSegmentedColormap.from_list("memory_ratio", ["#dc2626", "#f8fafc", "#16a34a"]).copy()
    cmap.set_bad("#e5e7eb")
    ...
    fig.suptitle("Grouped memory ratio over not-grouped", fontsize=16, fontweight="bold", y=0.98)
    ...
    cbar.set_ticklabels([format_ratio(x) for x in ticks])
    cbar.set_label("ratio = not-grouped RAM / grouped RAM")
    fig.text(
        0.5,
        0.06,
        "Values >1x mean grouped uses less RAM. RAM is averaged over the last 2 successful runs. One-sided OOM/OOT is '-'; both OOM/OOT keep that status.",
        ha="center",
        fontsize=9,
        color="#475569",
    )
